...
class AT_TextEdit(AT_IApp, AT_ITransform):
    def __init__(self):
        ...
        self.__drawText.setColor(self.__defaultTextColor)
        self.__drawText.setText(self.__textInfos.getText())
        ...
        self.__rectangleTextEdit.setSize(Size2D(200, 60))
        ...
        self.__confineText()
        self.__cursor = Rect(5, 5, 2, self.__drawText.getSize().getH())
    ...
    def __confineText(self):
        ...
        if self.__drawText.getSize().getH() >= self.__rectangleTextEdit.getSize().getH():
        ...
        y = self.__rectangleTextEdit.getPosition().getY() + (self.__rectangleTextEdit.getSize().getH() - self.__drawText.getSize().getH())/2

        self.__drawText.setPosition(Vector2D(x, y))

    def __recalculateCursorPosition(self, cursorPosition):
        x1 = 0
        if self.__contentWrite and self.__rectangleTextEdit.getSize().getW() - 10 < self.__drawText.getSize().getW():
            x1 = self.__drawText.getSize().getW() - self.__rectangleTextEdit.getSize().getW() + 10
        ...
        if cursorPosition == 0:
            ...
            w1 = self.__drawText.getWidth(len(self.__drawText.getText()) - 1)
            if self.__contentWrite == False or w1 == None:
                w1 = 0
            if cursorPosition == len(self.__drawText.getText()):
                if w1 >= self.__rectangleTextEdit.getSize().getW() - 10:
                    w1 = self.__rectangleTextEdit.getSize().getW() - 10
            else:
                w2 = self.__drawText.getWidth(cursorPosition - 1)
                w1 = w2 - x1
                if w1 < 0:
                    w1 = 0
        ...
        self.__cursor.y = self.__drawText.getPosition().getY()
        self.__cursor.height = self.__drawText.getSize().getH()

        self.__drawText.setArea(x1, 0, self.__rectangleTextEdit.getSize().getW() - 10,
    ...
    def updateEvent(self, event):
            ...
            if event.button == 1:
                if self.__rectangleTextEdit.isContain(Vector2D(event.pos[0], event.pos[1])) == True:
                    ...
                    if self.__contentWrite == False:
                        ...
                        self.__drawText.setText(text)
                    else:
                        xi = self.__rectangleTextEdit.getPosition().getX() -
                else:
                    action = "normal"
                    self.__writent = False
                    if len(self.__textInfos.getText()) <= 0:
                        self.__textInfos.setText(self.__defaultText)
                        self.__drawText.setText(self.__defaultText)
                        self.__contentWrite = False
        ...
        if event.type == KEYDOWN:
            if self.__writent:
                ...
                if event.key == K_BACKSPACE:
                    ...
                    self.__caracterCurseurPosition += 1
                    if self.__caracterCurseurPosition > len(self.__drawText.getText()):
                        self.__caracterCurseurPosition = len(self.__drawText.getText())
                elif event.key == K_UP:
                    self.__caracterCurseurPosition = len(self.__drawText.getText())
                    ...
                    if caracter in printable:
                        #if len(text1) == len(self.__drawText.getText()):
                        self.__caracterCurseurPosition += 1
                        text1 += caracter
                        text = text1 + text2
        ...
        if self.__contentWrite == True:
            if self.__textInfos.getColor() != self.__normalTextColor:
                ...
                self.__drawText.setColor(self.__normalTextColor)
